...
class Reminder {
	...
	public function __construct( \WP_Post $post ) {
		...
		if ( $post->post_type !== CPT::TYPE ) {
			throw new \InvalidArgumentException( __( "Invalid post type for post.", Plugin::SLUG ) );
		}
		...
		if ( in_array( $boa = get_post_meta( $post->ID, '_itelic_renewal_reminder_boa', true ), array(
			...
			throw new \InvalidArgumentException( __( "Invalid value for before or after.", Plugin::SLUG ) );
		}
